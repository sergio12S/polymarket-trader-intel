...
@mcp.tool()
def list_leaderboard_traders(period: str = "WEEK", limit: int = 25) -> dict:
    """Pull Polymarket PnL leaderboard into the local SQLite DB and return wallet list."""
    return T.list_leaderboard_traders(period=period, limit=limit)


@mcp.tool()
def scan_top_traders(period: str = "WEEK", limit: int = 12, max_trades: int = 300) -> dict:
    """Pull weekly/monthly top traders and download their recent trades/activity."""
...
def discover_btc15m_earners(scan_trades: int = 3000, limit: int = 15, max_trades: int = 300) -> dict:
    """CORE 1: Find wallets active on BTC 15m by scanning recent global trades (not PnL leaderboard)."""
    return T.discover_btc15m_earners(scan_trades=scan_trades, limit=limit, max_trades=max_trades)


@mcp.tool()
def btc15m_closed_pnl(wallet: str, max_rows: int = 400) -> dict:
...
@mcp.tool()
def pull_trader(wallet: str, max_trades: int = 400) -> dict:
    """Fetch trades and activity for one Polymarket proxyWallet into SQLite."""
    return T.pull_trader(wallet=wallet, max_trades=max_trades)


@mcp.tool()
def explain_trader_style(wallet: str) -> dict:
    """BTC 15m style card: timing in window, side/outcome mix, avg entry, style label (secondary)."""
    return T.explain_trader_style(wallet=wallet)


@mcp.tool()
def find_btc15m_earners(min_btc_trades: int = 10, limit: int = 20) -> list:
    """Rank already-pulled traders by BTC 15m focus (secondary; prefer find_stable_btc15m_earners)."""
    return T.find_btc15m_earners(min_btc_trades=min_btc_trades, limit=limit)


@mcp.tool()
def tool_catalog() -> list:
    """List trader-intel MCP tools and short descriptions."""
